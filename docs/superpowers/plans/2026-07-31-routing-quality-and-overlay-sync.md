...
1. **Doorway stubs extend along the opening's line axis, not its normal.** The venue data (JR East-derived) digitizes openings as *connector lines spanning the gap between spaces* — the line direction IS the walking direction (verified in existing fixtures, e.g. `opening_connected_blobs_skip_the_near_blob_bridge` where the door line crosses the gap perpendicular to the corridor walls). "Straight from the front" = collinear with the opening axis.
2. **`examples/analyze_synth.rs` is not extended.** Chord counts have no public reporting surface (warnings are user-facing and code-gated); the chord pass is fully covered by unit tests on `pub(crate) fn shortcut_chords` instead.
3. **No per-node chord cap.** The spec proposed ≤ 4 chords per node. Because each added chord shortens the graph paths used by the savings test for later candidates, chord degrees self-limit to ~2 (verified on a 10-node zigzag): the cap is unreachable dead code. Dropped; the savings test itself is the guard against redundant chords.
