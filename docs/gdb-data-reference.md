...
- Sections `9` (scene sources), `10` (canonical graph), `11` (network QA) are declared format ids with dependency edges onto §8. **§9 is implemented** (3D Stage 1): compiled generated-3D semantic primitives in venue-local millimetres (surfaces, walls, ceilings, portals as explicit topology relationships, and neutral/source-evidenced conveyance forms) plus the typed tiles descriptor slot (hashes, activation state, floor mappings, source-object associations, contextual classifications — never a URL or GLB bytes). §9's references (level membership, confidence, source locators, evidence) resolve into §8's registries — the first cross-section validation — and a present §9 whose §8 is unavailable reports `disabledByDependency`. §10 and §11 remain declared-only: a present row is never interpreted by a decoder that predates them.
