...
- The level table prints `resolvedPlaneM` beside `metadataElevationM` and their difference, which puts #31's 3.02 m KITTE disagreement in front of the person who exported it. Zero samples renders as "no residuals could be measured", never as a row of `0.00 m` — nothing mapped, so nothing was measured, and a clean-looking table would say the opposite.
- `e2e/tiles.producer.spec.ts` drives the loop in a browser and ends on the viewer's badge reading `3D Tiles`. Its package is built at a plane **no venue level sits near**: a few metres off passed every gate, because it mapped to whichever floor happened to sit there — wrongly and undetectably, which is the hazard #74's refusal to infer a datum exists to prevent. Package builders live in `tests/fixtures/` so the server and browser suites build the same bytes.
